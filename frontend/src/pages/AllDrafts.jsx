    ...
    <div className={`listing-card ${isDraft ? 'draft-card' : 'active-card'}`} onClick={onClick}>
      ...
      <div className="listing-details">
        ...
        <div className="listing-footer">
          ...
          <span className="listing-status status-draft">Draft</span>
        </div>
      </div>
    </div>
  );
...
function AllDrafts() {
  ...
  const fetchDrafts = async () => {
    try {
      ...
      const draftItems = response.data.filter(item => item.status === 'draft');
  ...
  if (loading) return <div className="all-drafts-container"><div className="loading">Loading drafts...</div></div>;
  if (error) return <div className="all-drafts-container"><div className="error">{error}</div></div>;

  return (
    <div className="all-drafts-container">
      <div className="page-header">
        ...
        <div className="header-content">
          <h1>📝 All Drafts</h1>
          <p className="header-subtitle">{drafts.length} draft{drafts.length !== 1 ? 's' : ''} saved</p>
        </div>
      </div>
